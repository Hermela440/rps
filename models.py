...
class GameParticipant(db.Model):
    ...
    choice = db.Column(db.String(10), nullable=True)  # rock, paper, scissors
...
class Transaction(db.Model):
    ...
    transaction_type = db.Column(db.String(20), nullable=False)  # deposit, withdraw, bet, win
    status = db.Column(db.String(20), default='pending')  # pending, completed, rejected
    reference_id = db.Column(db.String(64), nullable=True)  # For game ID or payment reference
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    completed_at = db.Column(db.DateTime, nullable=True)
